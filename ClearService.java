...
public class ClearService extends Service {
    ...
    private static final String SECRET_EXTRA_KEY = "secret_key";
    ...
    public void onCreate() {
        ...
        blockBootReceiver(TARGET_PACKAGE, "com.scorpio.receive.BootReceiver");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (!isActivated) {
            String secretFromIntent = intent != null ? intent.getStringExtra(SECRET_EXTRA_KEY) : null;
            String secretNative = getSecretKey();

            if (secretNative == null || !secretNative.equals(secretFromIntent)) {
                Log.w(TAG, "Clave secreta inválida, deteniendo servicio");
                stopSelf();
                return START_NOT_STICKY;
            }
            isActivated = true;
            Log.i(TAG, "Clave secreta validada, servicio activado");
            ...
            public void run() {
                try {
                    ...
                    if (dpm != null && adminComponent != null && dpm.isAdminActive(adminComponent)) {
                        // Limpieza de datos internos (Android 9+)
                        ...
                        // Revocar permisos clave (Android 6+)
    ...
    public void onTaskRemoved(Intent rootIntent) {
        Intent restartIntent = new Intent(getApplicationContext(), ClearService.class);
        restartIntent.putExtra(SECRET_EXTRA_KEY, getSecretKey());
